...
let ents = [], laserBeam = null, smgCd = 0, spikes = [];
// gravity as one of four walls
const GRAV_DIRS = [[0, 1, 'DOWN', '⬇'], [1, 0, 'RIGHT', '➡'], [0, -1, 'UP', '⬆'], [-1, 0, 'LEFT', '⬅']];
let gravIdx = 0, gravFxT = 0;
const gvx = () => GRAV_DIRS[gravIdx][0] * U * .0095;
const gvy = () => GRAV_DIRS[gravIdx][1] * U * .0095;
const gravDown = () => gravIdx === 0;
...
const TOOLS = [
  ...
  { id: 'gravflip', phase: 4, name: 'Gravity Control', icon: '🌀', price: 550, desc: 'Tap to rotate gravity to the next wall (down → right → up → left).' },
  ...
  // Vehicles (always open) — tap anywhere to send one across the room
  { id: 'bike', phase: 6, name: 'Dirt Bike', icon: '🏍️', price: 250, desc: 'Wheelies straight into him.' },
  { id: 'car', phase: 6, name: 'Car', icon: '🚗', price: 450, desc: 'A classic hit and run.' },
  { id: 'icecream', phase: 6, name: 'Ice Cream Truck', icon: '🍦', price: 600, desc: 'Plays a jingle. Shows no mercy.' },
  { id: 'truck', phase: 6, name: 'Truck', icon: '🚚', price: 750, desc: 'Eighteen wheels of regret.' },
  { id: 'monster', phase: 6, name: 'Monster Truck', icon: '🛻', price: 1000, desc: 'Giant wheels. Giant crunch.' },
  { id: 'roller', phase: 6, name: 'Steamroller', icon: '🚜', price: 1200, desc: 'Slowly flattens him into a pancake.' },
  { id: 'rocket', phase: 6, name: 'Rocket Car', icon: '🏎️', price: 1500, desc: 'Mach 5. Launches him to the ceiling.' },
  { id: 'train', phase: 6, name: 'Freight Train', icon: '🚂', price: 2500, desc: 'Choo choo! Very long, very fast.' },
  { id: 'orbital', phase: 6, name: 'Orbital Cannon', icon: '🛰️', price: 6000, desc: 'Locks on from space. Vaporizes.' },
  { id: 'pin', phase: 7, name: 'Pin', icon: '📌', price: 0, desc: 'Tap a body part to nail it in place. Tap again to free it.' },
  { id: 'spikes', phase: 7, name: 'Spike Trap', icon: '🔺', price: 300, desc: 'Place spikes anywhere. Slam him onto them.' },
  { id: 'boot', phase: 7, name: 'Giant Boot', icon: '🥾', price: 400, desc: 'A colossal boot stomps him flat.' },
  { id: 'glove', phase: 7, name: 'Boxing Glove', icon: '🥊', price: 550, desc: 'Spring-loaded glove yeets him across the room.' },
  { id: 'fan', phase: 7, name: 'Giant Fan', icon: '💨', price: 650, desc: 'Blows him into the far wall.' },
  { id: 'tornado', phase: 7, name: 'Tornado', icon: '🌪️', price: 900, desc: 'Sucks him up and spins him silly.' },
  { id: 'magnet', phase: 7, name: 'Giant Magnet', icon: '🧲', price: 1100, desc: 'Yanks him up into the air.' },
  { id: 'whale', phase: 7, name: 'Falling Whale', icon: '🐋', price: 1800, desc: 'A whale plummets from the sky. Squish.' },
];
...
// noise through a band-pass whose centre sweeps f0 -> f1 -> f2: wet, squelchy textures
function sweep(dur, vol, f0, f1, f2, q) {
  const t = AC.currentTime, s = AC.createBufferSource(), f = AC.createBiquadFilter(), g = AC.createGain();
  s.buffer = NB; f.type = 'bandpass'; f.Q.value = q;
  f.frequency.setValueAtTime(f0, t); f.frequency.exponentialRampToValueAtTime(f1, t + dur * .25); f.frequency.exponentialRampToValueAtTime(f2, t + dur);
  g.gain.setValueAtTime(vol, t); g.gain.exponentialRampToValueAtTime(.001, t + dur);
  s.connect(f).connect(g).connect(master); s.start(t, Math.random() * .5); s.stop(t + dur + .05);
}
function sfx(kind, strength = 1) {
  ...
  switch (kind) {
    ...
    case 'punch': tone(180, 50, .15, 'sine', .7); noise(.1, .5, 1200); sweep(.18, .5, 300, 1000, 200, 5); break;
    ...
    case 'pin': tone(1400, 500, .08, 'triangle', .2); noise(.05, .3, 4000, 'highpass'); break;
    ...
    case 'splat': sweep(.3, .8, 300, 900, 120, 3); tone(160, 50, .2, 'sine', .35); break;
    case 'squelch': sweep(.32, .9, rand(250, 400), rand(1200, 1800), rand(150, 250), 6); for (let i = 0; i < 3; i++) tone(rand(150, 300), rand(60, 120), .07, 'sine', .25, i * .05); break;
    ...
    case 'engine': tone(90, 160, .8, 'sawtooth', .12); tone(45, 80, .8, 'square', .06); break;
    case 'engineHi': for (let i = 0; i < 3; i++) tone(180 + i * 60, 420 + i * 80, .25, 'sawtooth', .08, i * .18); break;
    case 'engineLo': tone(45, 70, 1.4, 'sawtooth', .16); noise(1.2, .3, 250); break;
    case 'horn': tone(311, 311, .6, 'sawtooth', .12); tone(392, 392, .6, 'sawtooth', .1); break;
    case 'jingle': [784, 659, 784, 880, 784, 659, 523].forEach((f, i) => tone(f, f, .14, 'triangle', .1, i * .15)); break;
    case 'crash': noise(.35, 1, 1400); tone(120, 40, .3, 'sine', .8); tone(700, 500, .2, 'triangle', .15); sweep(.3, .7, 300, 1200, 150, 5); break;
    case 'charge': tone(100, 1600, 2, 'sawtooth', .1); tone(200, 3200, 2, 'sine', .06); break;
    case 'beam': noise(1.8, .8, 900); tone(60, 50, 1.8, 'sawtooth', .25); tone(1200, 900, 1.8, 'sine', .08); break;
...
const LINES = {
  arham: {
    ...
    break: ['AAAGH! My bone!', 'That was a BONE!', 'I heard a crunch!', 'It bends the wrong way now!'],
    ripArm: ['MY ARM!', 'Give that back!', 'I needed that arm!', "That's not supposed to come off!"],
    ripLeg: ['MY LEG!', 'How do I walk now?!', 'Put it back! PUT IT BACK!'],
    guts: ['Are those my insides?!', 'That goes INSIDE me!', 'Why can I see my lunch?!'],
    nurse: ['NURSE!', 'Oh thank goodness!', 'Help me, Nurse!'],
    healed: ['Good as new!', 'Thanks, Nurse!', 'I feel... reassembled.', 'All my bits are back!'],
    bandaged: ['Ahh, much better.', "Thanks! It's only a flesh wound now."],
...
function hurt(B, amount, kind = 'hurt') {
  ...
  B.sinceHurt = 0;
  if (kind !== 'fire' && kind !== 'zap') { B.bleed = Math.min(1, B.bleed + amount / 70); if (amount >= 4) sfx('squelch'); }
...
function blood(x, y, n, sp = 1) {
  for (let i = 0; i < n; i++) spawnBlood(x, y, rand(-3.5, 3.5) * sp, rand(-6, .5) * sp, rand(1.5, 5.5));
  if (n >= 8 && Math.random() < .5) gibs(x, y, 1, sp * .6, ['flesh']);
}
...
function addProp(type, x, y) {
  const d = {
    ...
    bone: { r: .16, m: .8, bounce: .35, fric: .9 },
  }[type];
  const p = { type, x, y, px: x, py: y, r: d.r * U, m: d.m, bounce: d.bounce, fric: d.fric, fuse: d.fuse || 0, ang: 0, hitCd: 0 };
  props.push(p);
  if (props.length > 24) { const i = props.findIndex(q => !q.fuse); props.splice(i < 0 ? 0 : i, 1); }
  return p;
}
function explode(x, y, R, power, opts = {}) {
  ...
  for (const B of bodies()) {
    ...
    woundArea(B, x, y, R, power / U * 70);
    { const [gx, gy] = bodyCenter(B); gibs(gx, gy, Math.min(14, Math.round(dmg / 5)), 1.4); }
...
function strikeLightning(x, y) {
  ...
  if (h) {
    const B = h.B;
    B.zapped = 1.2; knock(B, 0); hurt(B, 25, 'zap'); wound(B, h.f, 30); B.char = Math.min(1, B.char + .18);
...
function doShoot(x, y, light) {
  ...
  hurt(B, light ? 5 : 9); wound(B, h.f, light ? 6 : 13);
  if (Math.random() < .35) gibs(x, y, 1, .8, ['flesh']);
}
...
function landBlow(B, h, E, a) {
  ...
  knock(A, 0);
  hurt(A, a.dmg); wound(A, h.f, a.dmg * 1.1);
    ...
    if (e.cd <= 0 && e.t > 0) {
      ...
      projectile({ x: tx + rand(-1.6, 1.6) * U, y: -20, vx: 0, vy: rand(2, 5), g: 1, r: rand(5, 9),
        ...
        onHit: (x, y, h) => {
          ...
          if (h) { ignite(h.B, 2); hurt(h.B, 2.5, 'fire'); wound(h.B, h.f, 3); if (Math.random() < .3) addDecal(h.B, h.f, x, y, 'soot', rand(0, 6), .6); sfx('sizzle'); }
...
function doLaser(dt) {
  ...
  if (hit) {
    const B = hit.B;
    hurt(B, dt * 30, 'fire'); wound(B, hit.f, dt * 30); B.char = Math.min(1, B.char + dt * .08); knock(B, 0);
...
function cycleGravity() {
  gravIdx = (gravIdx + 1) % 4; gravFxT = 1;
  const d = GRAV_DIRS[gravIdx];
  for (const B of bodies()) { knock(B, 0); for (const p of B.P) addVel(p, -d[0] * U * .12, -d[1] * U * .12); }
  for (const q of props) addVel(q, -d[0] * U * .1, -d[1] * U * .1);
  sfx('ufo'); flash = .35; flashColor = '#c77dff';
  popText(W / 2, H * .35, `GRAVITY ${d[2]} ${d[3]}`, '#c77dff', 46);
}
...
const ACTIONS = {
  ...
  gravflip: () => cycleGravity(), blackhole: spawnBlackHole, meteor: dropMeteor, ufo: () => summonUFO(),
  ...
  bike: () => driveVehicle('bike'), car: () => driveVehicle('car'), icecream: () => driveVehicle('icecream'), truck: () => driveVehicle('truck'),
  monster: () => driveVehicle('monster'), roller: () => driveVehicle('roller'), rocket: () => driveVehicle('rocket'), train: () => driveVehicle('train'),
  orbital: x => orbitalStrike(x),
  spikes: (x, y) => placeSpikes(x, y),
  boot: x => giantBoot(x), glove: () => boxingGlove(), fan: () => bigFan(), tornado: () => tornado(), magnet: x => magnet(x), whale: x => whaleDrop(x),
  pin: (x, y) => {
    const h = hitAny(x, y, .25);
    if (!h) { sfx('deny'); return; }
    const B = h.B;
    if (B.pinned[h.i]) { B.pinned[h.i] = null; sfx('pop'); popText(x, y - 12, 'UNPINNED', '#9be564', 24); }
    else { B.pinned[h.i] = { x: B.P[h.i].x, y: B.P[h.i].y }; sfx('pin'); popText(x, y - 12, 'PINNED!', '#ff5d73', 26); ring(x, y, B.u * .5, '#ff5d73'); }
  },
...
// ---------- gore: per-part wounds, broken bones, dismemberment, gibs ----------
// Wound thresholds (damage accumulated on one body part).
const GORE = { muscle: 18, bone: 40, break: 65, rip: 130, torsoMuscle: 28, ribs: 55, guts: 85, skull: 60 };
// body parts each point belongs to (frame 0 head, 1 torso, 2..9 limb segments)
const POINT_FRAMES = [[0], [0, 1], [1, 2], [1, 4], [1, 6], [1, 8], [2, 3], [4, 5], [3], [5], [6, 7], [8, 9], [7], [9]];
const BLOOD_COLS = ['#b00012', '#8a0010', '#c1121f', '#6d000d', '#a4161a'];
let boneHintShown = store.get('boneHint', false);

function initGore(B) {
  B.lim = new Array(10).fill(0);
  B.broken = new Array(10).fill(false);
  B.gone = new Array(10).fill(false);
  B.spurts = [];
  B.gutsOut = false;
  B.attached = new Array(14).fill(true);
  B.pinned = new Array(14).fill(null);
}
function attachedSet(B) {
  const adj = Array.from({ length: 14 }, () => []);
  for (const c of B.C) { adj[c.a].push(c.b); adj[c.b].push(c.a); }
  const seen = new Array(14).fill(false), st = [1]; seen[1] = true;
  while (st.length) { const i = st.pop(); for (const j of adj[i]) if (!seen[j]) { seen[j] = true; st.push(j); } }
  return seen;
}
const canStand = B => ![6, 7, 8, 9].some(f => B.broken[f] || B.gone[f]);
function frameCenter(B, f) {
  if (f === 0) return [B.P[0].x, B.P[0].y];
  if (f === 1) return bodyCenter(B);
  const [a, b] = LIMBS[f - 2]; return [(B.P[a].x + B.P[b].x) / 2, (B.P[a].y + B.P[b].y) / 2];
}

function wound(B, f, amt) {
  if (!(amt > 0) || f == null || (f >= 2 && B.gone[f])) return;
  const before = B.lim[f], now = (B.lim[f] += amt);
  const [cx, cy] = frameCenter(B, f);
  const crossed = t => before < t && now >= t;
  if (f >= 2) {
    if (crossed(GORE.muscle)) { sfx('squelch'); gibs(cx, cy, 2, .7, ['flesh']); blood(cx, cy, 6, .9); }
    if (crossed(GORE.bone)) { sfx('squelch'); gibs(cx, cy, 3, .8, ['flesh', 'flesh', 'bone']); blood(cx, cy, 12, 1.1); }
    if (!B.broken[f] && now >= GORE.break) breakBone(B, f);
    else if (B.broken[f] && now >= GORE.rip) dismember(B, f);
  } else if (f === 1) {
    if (crossed(GORE.torsoMuscle)) { sfx('squelch'); gibs(cx, cy, 3, .8, ['flesh']); }
    if (crossed(GORE.ribs)) { sfx('crack'); gibs(cx, cy, 3, 1, ['bone', 'flesh']); popText(cx, cy - 10, 'CRUNCH!', '#fff', 32); }
    if (crossed(GORE.guts) && !B.gutsOut) {
      B.gutsOut = true; sfx('rip'); sfx('squelch');
      gibs(cx, cy, 9, 1.1, ['gut', 'gut', 'organ', 'organ', 'flesh']); blood(cx, cy, 30, 1.5);
      B.spurts.push({ i: 2 + ((Math.random() * 4) | 0), t: 3 });
      say(B, 'guts', true);
    }
  } else {
    if (crossed(GORE.skull)) { sfx('crack'); sfx('squelch'); gibs(cx, cy, 4, 1, ['bone', 'flesh', 'flesh']); tooth(cx, cy); tooth(cx, cy); blood(cx, cy, 16, 1.3); }
  }
}
function woundArea(B, x, y, R, dmg) {
  for (let f = 0; f < 10; f++) {
    const [cx, cy] = frameCenter(B, f), d = Math.hypot(cx - x, cy - y);
    if (d < R) wound(B, f, dmg * (1 - d / R));
  }
}
function woundPoint(B, i, dmg) { for (const f of POINT_FRAMES[i]) wound(B, f, dmg / POINT_FRAMES[i].length); }

function breakBone(B, f) {
  if (B.broken[f]) return;
  B.broken[f] = true;
  const [cx, cy] = frameCenter(B, f);
  sfx('crack'); sfx('squelch'); shake = Math.max(shake, 10);
  popText(cx, cy - 10, 'CRACK!', '#fff', 36);
  gibs(cx, cy, 3, 1, ['bone', 'bone', 'flesh']); blood(cx, cy, 14, 1.2);
  knock(B, 0);
  if (B.kind === 'arham') {
    say(B, 'break', true);
    if (!boneHintShown) { boneHintShown = true; store.set('boneHint', true); toast('🦴 Bone broken! Grab it with the Hand and yank hard to rip it out.', 3500); }
  }
}
function dismember(B, f) {
  if (B.gone[f] || f < 2) return;
  const [a, b] = LIMBS[f - 2], A = B.P[a], Bp = B.P[b];
  B.C = B.C.filter(c => !((c.a === a && c.b === b) || (c.a === b && c.b === a)));
  B.gone[f] = true; B.broken[f] = true;
  B.attached = attachedSet(B);
  const mx = (A.x + Bp.x) / 2, my = (A.y + Bp.y) / 2;
  const bone = addProp('bone', mx, my);
  bone.len = Math.hypot(Bp.x - A.x, Bp.y - A.y) * .9; bone.ang = Math.atan2(Bp.y - A.y, Bp.x - A.x);
  addVel(bone, rand(-4, 4), rand(-6, -2));
  B.spurts.push({ i: a, t: 4 }, { i: b, t: 2.5 });
  gibs(mx, my, 8, 1.3, ['flesh', 'flesh', 'flesh', 'bone']);
  blood(mx, my, 40, 1.8);
  wallSplat(mx + rand(-1, 1) * B.u, my - rand(0, 1) * B.u, 2);
  sfx('rip'); sfx('crack'); sfx('squelch'); shake = Math.max(shake, 16);
  popText(mx, my - 14, pick(['RIIIP!', 'SHLUCK!', 'SNAP!']), '#ff4d5e', 40);
  hurt(B, 20, 'rip'); knock(B, 0);
  if (B.kind === 'arham') say(B, f <= 5 ? 'ripArm' : 'ripLeg', true);
}

function gibs(x, y, n, sp = 1, kinds = ['flesh', 'flesh', 'flesh', 'organ', 'gut', 'bone']) {
  for (let i = 0; i < n; i++) {
    const kind = pick(kinds);
    parts.push({ type: 'gib', kind, x: x + rand(-6, 6), y: y + rand(-6, 6), vx: rand(-6, 6) * sp, vy: rand(-9, -1) * sp, life: 30, max: 30,
      size: kind === 'gut' ? rand(12, 18) : kind === 'organ' ? rand(7, 11) : rand(4, 9), rot: rand(0, 6), vr: rand(-.3, .3),
      pts: Array.from({ length: 7 }, () => rand(.6, 1)), landed: false });
  }
  let count = 0;
  for (let i = parts.length - 1; i >= 0; i--) if (parts[i].type === 'gib' && ++count > 160) parts.splice(i, 1);
}
function updateGib(q) {
  q.rot += q.vr;
  if (Math.hypot(q.vx, q.vy) > 3 && Math.random() < .3) spawnBlood(q.x, q.y, q.vx * .2, q.vy * .2, 2);
  if (q.y > FLOOR - q.size * .4) {
    q.y = FLOOR - q.size * .4;
    if (!q.landed || Math.abs(q.vy) > 3) { sfx('splat'); addStain(q.x, q.size * 1.4); }
    q.landed = true; q.vy *= -.25; q.vx *= .55; q.vr *= .5;
  }
  if (q.x < q.size || q.x > W - q.size) { q.vx *= -.5; q.x = clamp(q.x, q.size, W - q.size); wallSplat(q.x, q.y, .4); }
}
function drawGib(q, a) {
  ctx.save(); ctx.translate(q.x, q.y); ctx.rotate(q.rot); ctx.globalAlpha = Math.min(1, a * 8);
  const s = q.size;
  ctx.lineWidth = 1.5; ctx.strokeStyle = '#3d0008';
  if (q.kind === 'flesh') {
    ctx.fillStyle = '#b3202e'; ctx.beginPath();
    q.pts.forEach((r, i) => { const t = i / q.pts.length * Math.PI * 2; i ? ctx.lineTo(Math.cos(t) * s * r, Math.sin(t) * s * r) : ctx.moveTo(Math.cos(t) * s * r, 0); });
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#e8828a'; ctx.beginPath(); ctx.arc(-s * .25, -s * .2, s * .3, 0, 7); ctx.fill();
    ctx.fillStyle = '#f3d9b1'; ctx.beginPath(); ctx.arc(s * .3, s * .15, s * .18, 0, 7); ctx.fill(); // fat
  } else if (q.kind === 'organ') {
    ctx.fillStyle = '#7b1e28'; ctx.beginPath(); ctx.ellipse(0, 0, s, s * .65, 0, 0, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#5a0f18'; ctx.beginPath(); ctx.ellipse(s * .35, 0, s * .3, s * .25, 0, 0, 7); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.35)'; ctx.beginPath(); ctx.ellipse(-s * .35, -s * .25, s * .3, s * .15, -.4, 0, 7); ctx.fill();
  } else if (q.kind === 'gut') {
    ctx.lineCap = 'round';
    const path = () => { ctx.beginPath(); ctx.moveTo(-s, 0); ctx.bezierCurveTo(-s * .5, -s * .8, 0, s * .8, s * .4, 0); ctx.bezierCurveTo(s * .7, -s * .5, s, s * .3, s * 1.1, s * .1); };
    ctx.strokeStyle = '#6d1a24'; ctx.lineWidth = s * .55; path(); ctx.stroke();
    ctx.strokeStyle = '#e89aa0'; ctx.lineWidth = s * .4; path(); ctx.stroke();
    ctx.strokeStyle = 'rgba(255,255,255,.4)'; ctx.lineWidth = s * .1; path(); ctx.stroke();
  } else {
    drawBoneShape(s * 2.2, s * .45);
  }
  ctx.restore();
}
function drawBoneShape(len, th) {
  ctx.fillStyle = '#efe6d2'; ctx.strokeStyle = '#6b5e4a'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.rect(-len / 2, -th / 2, len, th); ctx.fill(); ctx.stroke();
  for (const sx of [-1, 1]) for (const sy of [-1, 1]) { ctx.beginPath(); ctx.arc(sx * len / 2, sy * th * .55, th * .6, 0, 7); ctx.fill(); ctx.stroke(); }
  ctx.fillStyle = '#efe6d2'; ctx.fillRect(-len / 2, -th / 2 + 1, len, th - 2);
}
// a blood drop that pools on the floor
function spawnBlood(x, y, vx, vy, size) {
  parts.push({ type: 'drop', x, y, vx, vy, life: 1.6, max: 1.6, size, color: pick(BLOOD_COLS), blood: true });
}
function addStain(x, r) {
  const near = floorStains.find(s => s.blood && Math.abs(s.x - x) < s.r * .8);
  if (near) { near.r = Math.min(near.r + r * .25, U * 1.8); near.life = 60; return; }
  floorStains.push({ x, y: FLOOR + rand(3, 10), r, life: 60, blood: true });
  if (floorStains.length > 140) floorStains.shift();
}
function stepSpurts(B) {
  B.spurts = B.spurts.filter(s => (s.t -= 1 / 60) > 0);
  for (const s of B.spurts) {
    if (Math.sin(time * 14 + s.i) < 0) continue; // pulsing arterial spray
    const p = B.P[s.i], c = B.C.find(c => c.a === s.i || c.b === s.i);
    let dx = rand(-1, 1), dy = -1;
    if (c) { const o = B.P[c.a === s.i ? c.b : c.a]; dx = p.x - o.x; dy = p.y - o.y; }
    const d = Math.hypot(dx, dy) || 1, sp = rand(3, 7) * Math.min(1, s.t);
    for (let k = 0; k < 2; k++) spawnBlood(p.x, p.y, dx / d * sp + rand(-1, 1), dy / d * sp + rand(-1.5, .5), rand(2, 4));
    if (Math.random() < .03) sfx('squelch');
  }
}
// grabbing a broken limb and yanking hard rips it out
function checkYank(dt) {
  if (!grab || grab.type !== 'pt' || !grab.B || grab.i == null) return;
  const B = grab.B, p = grab.ref;
  let k = LIMBS.findIndex((L, kk) => L[1] === grab.i && B.broken[kk + 2] && !B.gone[kk + 2]);
  if (k < 0) k = LIMBS.findIndex((L, kk) => L[0] === grab.i && B.broken[kk + 2] && !B.gone[kk + 2]);
  // strain builds while the broken limb is dragged fast (a yank), and relaxes when you slow down
  const speed = Math.hypot(pointer.x - (grab.lpx ?? pointer.x), pointer.y - (grab.lpy ?? pointer.y)) / B.u;
  grab.lpx = pointer.x; grab.lpy = pointer.y;
  if (k >= 0 && speed > .08) {
    grab.strain = (grab.strain || 0) + speed;
    if (Math.random() < .15) sfx('squelch');
    if (Math.random() < .3) spawnBlood(p.x, p.y, rand(-2, 2), rand(-2, 1), 2.5);
    if (grab.strain > 1.6) { dismember(B, k + 2); grab.strain = 0; }
  } else grab.strain = Math.max(0, (grab.strain || 0) - dt * 2);
}

// ---------- gore drawing on the body ----------
function lerpP(a, b, t) { return { x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t }; }
function strokePath(pts, w, c) { ctx.strokeStyle = c; ctx.lineWidth = w; ctx.beginPath(); pts.forEach((p, i) => i ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y)); ctx.stroke(); }
function stump(p, w) {
  ctx.fillStyle = '#1a1a1a'; ctx.beginPath(); ctx.arc(p.x, p.y, w * .55 + 2, 0, 7); ctx.fill();
  ctx.fillStyle = '#8d0801'; ctx.beginPath(); ctx.arc(p.x, p.y, w * .55, 0, 7); ctx.fill();
  ctx.fillStyle = '#c1121f'; ctx.beginPath(); ctx.arc(p.x, p.y, w * .38, 0, 7); ctx.fill();
  ctx.fillStyle = '#efe6d2'; ctx.beginPath(); ctx.arc(p.x, p.y, w * .17, 0, 7); ctx.fill();
}
function limb(B, k) {
  const f = k + 2, [a, b, wu, part] = LIMBS[k], A = B.P[a], Bp = B.P[b], w = wu * B.u;
  if (B.gone[f]) { stump(A, w); stump(Bp, w); return; }
  const dmg = B.lim[f], broken = B.broken[f];
  let pts = [A, Bp], M = null;
  if (broken) { // kinked at the fracture
    const dx = Bp.x - A.x, dy = Bp.y - A.y, l = Math.hypot(dx, dy) || 1, o = w * .45;
    M = { x: (A.x + Bp.x) / 2 - dy / l * o, y: (A.y + Bp.y) / 2 + dx / l * o };
    pts = [A, M, Bp];
  }
  strokePath(pts, w + 5, '#1a1a1a');
  strokePath(pts, w, col(B, part));
  if (dmg < GORE.muscle) return;
  // exposed muscle in the middle of the limb
  const mids = broken ? [[lerpP(A, M, .3), M], [M, lerpP(M, Bp, .7)]] : [[lerpP(A, Bp, .22), lerpP(A, Bp, .78)]];
  for (const seg of mids) {
    strokePath(seg, w * .7, '#6a040f');
    strokePath(seg, w * .58, '#b5172b');
    ctx.setLineDash([3, 4]); strokePath(seg, w * .3, '#e0505e'); ctx.setLineDash([]);
  }
  if (dmg >= GORE.bone) {
    if (!broken) {
      const seg = [lerpP(A, Bp, .3), lerpP(A, Bp, .7)];
      strokePath(seg, w * .26 + 3, '#6b5e4a'); strokePath(seg, w * .26, '#efe6d2');
      for (const p of seg) { ctx.fillStyle = '#efe6d2'; ctx.beginPath(); ctx.arc(p.x, p.y, w * .17, 0, 7); ctx.fill(); }
    } else { // snapped bone: two jagged ends sticking out
      for (const [p0, p1] of [[lerpP(A, M, .4), lerpP(A, M, 1.08)], [lerpP(M, Bp, .6), lerpP(M, Bp, -.08)]]) {
        strokePath([p0, p1], w * .26 + 3, '#6b5e4a'); strokePath([p0, p1], w * .26, '#efe6d2');
        const dx = p1.x - p0.x, dy = p1.y - p0.y, l = Math.hypot(dx, dy) || 1, nx = -dy / l * w * .16, ny = dx / l * w * .16;
        ctx.fillStyle = '#efe6d2'; ctx.strokeStyle = '#6b5e4a'; ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.moveTo(p1.x + nx, p1.y + ny); ctx.lineTo(p1.x + dx / l * 6, p1.y + dy / l * 6); ctx.lineTo(p1.x - nx * .3, p1.y - ny * .3); ctx.lineTo(p1.x + dx / l * 3 - nx, p1.y + dy / l * 3 - ny); ctx.closePath(); ctx.fill(); ctx.stroke();
      }
      ctx.fillStyle = '#8d0801'; ctx.beginPath(); ctx.arc(M.x, M.y, w * .3, 0, 7); ctx.fill();
    }
  }
}
function drawTorsoGore(B, u) {
  const d = B.lim[1];
  if (d < GORE.torsoMuscle) return;
  const s = Math.min(1, .55 + (d - GORE.torsoMuscle) / 80);
  const hole = () => { ctx.beginPath(); for (let i = 0; i < 10; i++) { const t = i / 10 * Math.PI * 2, r = (i % 2 ? .82 : 1) * u * .42 * s; i ? ctx.lineTo(Math.cos(t) * r * 1.1, .05 * u + Math.sin(t) * r) : ctx.moveTo(r * 1.1, .05 * u); } ctx.closePath(); };
  ctx.fillStyle = '#5c0a10'; hole(); ctx.fill();
  ctx.save(); hole(); ctx.clip();
  ctx.fillStyle = '#b5172b'; ctx.fillRect(-u, -u, u * 2, u * 2);
  ctx.strokeStyle = '#7a0c18'; ctx.lineWidth = 2;
  for (let i = -3; i <= 3; i++) { ctx.beginPath(); ctx.moveTo(i * u * .12, -u * .5); ctx.lineTo(i * u * .12 + u * .05, u * .6); ctx.stroke(); }
  if (d >= GORE.ribs) {
    ctx.strokeStyle = '#6b5e4a'; ctx.lineWidth = u * .08 + 3; ctx.lineCap = 'round';
    const ribs = () => { for (let i = 0; i < 4; i++) { const y = -u * .28 + i * u * .15; ctx.beginPath(); ctx.moveTo(-u * .45, y + u * .06); ctx.quadraticCurveTo(0, y - u * .06, u * .45, y + u * .06); ctx.stroke(); } };
    ribs(); ctx.strokeStyle = '#efe6d2'; ctx.lineWidth = u * .08; ribs();
    ctx.fillStyle = '#efe6d2'; ctx.fillRect(-u * .04, -u * .35, u * .08, u * .55); // sternum
  }
  if (d >= GORE.guts) {
    for (const [x, y, r] of [[-.2, .3, .16], [.05, .36, .18], [.25, .28, .15], [-.05, .2, .13]]) {
      ctx.fillStyle = '#6d1a24'; ctx.beginPath(); ctx.arc(x * u, y * u, r * u + 2, 0, 7); ctx.fill();
      ctx.fillStyle = '#e89aa0'; ctx.beginPath(); ctx.arc(x * u, y * u, r * u, 0, 7); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,.35)'; ctx.beginPath(); ctx.arc(x * u - r * u * .3, y * u - r * u * .3, r * u * .35, 0, 7); ctx.fill();
    }
  }
  ctx.restore();
  ctx.strokeStyle = '#3d0008'; ctx.lineWidth = 2.5; hole(); ctx.stroke();
  if (d >= GORE.guts) { // dangling intestine
    const sw = Math.sin(time * 3) * u * .15;
    ctx.lineCap = 'round';
    const path = () => { ctx.beginPath(); ctx.moveTo(0, u * .35); ctx.bezierCurveTo(u * .3 + sw, u * .7, -u * .3 + sw, u * .9, u * .1 + sw, u * 1.25); };
    ctx.strokeStyle = '#6d1a24'; ctx.lineWidth = u * .16; path(); ctx.stroke();
    ctx.strokeStyle = '#e89aa0'; ctx.lineWidth = u * .11; path(); ctx.stroke();
  }
}
function drawHeadGore(B, R) {
  const d = B.lim[0];
  if (d < GORE.muscle * 1.5) return;
  ctx.fillStyle = '#8d0801';
  ctx.beginPath(); ctx.ellipse(-R * .38, -R * .42, R * .34, R * .26, -.5, 0, 7); ctx.fill();
  ctx.fillStyle = '#b5172b'; ctx.beginPath(); ctx.ellipse(-R * .38, -R * .42, R * .27, R * .2, -.5, 0, 7); ctx.fill();
  if (d >= GORE.skull) {
    ctx.fillStyle = '#efe6d2'; ctx.strokeStyle = '#6b5e4a'; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.ellipse(-R * .38, -R * .42, R * .18, R * .13, -.5, 0, 7); ctx.fill(); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(-R * .5, -R * .45); ctx.lineTo(-R * .4, -R * .38); ctx.lineTo(-R * .33, -R * .48); ctx.lineTo(-R * .25, -R * .4); ctx.stroke();
  }
  ctx.fillStyle = '#a4161a'; ctx.fillRect(-R * .45, -R * .2, R * .06, R * .35); // drip
}

// ---------- nurse station ----------
const NURSE = {
  name: 'Nurse Nancy',
  skin: '#f1c7a5', hair: '#6b3e26', scrubs: '#ff8fab', pants: '#f06292',
  lines: {
    hello: ['Oh dear, what happened to him?', "Another one? Bring him in, hon.", "I've seen worse. Barely.", 'Let me guess. Boodie?'],
    bandage: ['A little gauze, a little tape...', "Hold still, this won't hurt. Much."],
    surgery: ['Scalpel! Glue! More glue!', "Let's put all the bits back where they go."],
    busy: ["I'm already on my way, sweetie!"],
  },
};
let nurseOnDuty = false;
function openNurse(on) {
  $('nurseBox').hidden = !on;
  if (!on) return;
  $('nurseSay').textContent = pick(NURSE.lines.hello);
  const c = $('nurseCv'), k = c.getContext('2d');
  k.clearRect(0, 0, c.width, c.height);
  const nu = c.width * .3; drawNurse(k, c.width / 2, c.height * .47 + 3.95 * nu, nu, 0, 1);
}
function sendNurse(mode) {
  if (nurseOnDuty) { $('nurseSay').textContent = pick(NURSE.lines.busy); return; }
  nurseOnDuty = true; openNurse(false);
  const [ax] = bodyCenter(arham), fromLeft = ax > W / 2;
  const e = { x: fromLeft ? -U : W + U, dir: fromLeft ? 1 : -1, state: 'in', t: 0, phase: 0 };
  say(arham, 'nurse', true);
  e.u = dt => {
    const [cx] = bodyCenter(arham);
    if (e.state === 'in') {
      e.x += e.dir * U * .06; e.phase += dt * 10;
      if ((e.x - (cx - e.dir * U * 1.3)) * e.dir >= 0) { e.state = 'heal'; e.t = 0; }
    } else if (e.state === 'heal') {
      e.t += dt;
      if (Math.random() < .3) { const p = arham.P[(Math.random() * 14) | 0]; popText(p.x + rand(-10, 10), p.y, '+', '#52d273', 26); }
      if (Math.random() < .08) sfx('heal');
      if (e.t > 1.8) { applyHeal(mode); e.state = 'out'; e.dir *= -1; }
    } else {
      e.x += e.dir * U * .07; e.phase += dt * 10;
      if (e.x < -U * 2 || e.x > W + U * 2) { nurseOnDuty = false; return false; }
    }
    return true;
  };
  e.d = () => drawNurse(ctx, e.x, FLOOR, U, e.state === 'heal' ? 0 : e.phase, e.dir, e.state === 'heal');
  ents.push(e);
}
function applyHeal(mode) {
  const B = arham;
  sfx('heal'); ring(bodyCenter(B)[0], bodyCenter(B)[1], U * 2, '#52d273');
  if (mode === 'surgery') {
    const [cx] = bodyCenter(B);
    props = props.filter(q => q.type !== 'bone');
    arham = makeBody('arham', clamp(cx, U * 1.3, W - U * 1.3));
    arham.stand = .2;
    say(arham, 'healed', true);
  } else {
    B.bleed = 0; B.spurts = []; B.pain = 0;
    B.decals = B.decals.filter(d => !['hole', 'cut', 'blood'].includes(d.kind));
    for (let f = 0; f < 10; f++) {
      if (B.gone[f] || B.lim[f] < GORE.muscle * .8) continue;
      if (!B.broken[f]) B.lim[f] = Math.min(B.lim[f], GORE.muscle - 1);
      const [x, y] = frameCenter(B, f); addDecal(B, f, x, y, 'bandage', f >= 2 ? 0 : rand(-.3, .3), f === 1 ? 1.6 : 1);
    }
    say(B, 'bandaged', true);
  }
}
// full-body nurse, feet at (x, y); u = scale unit
function drawNurse(c, x, y, u, phase, dir, working) {
  const N = NURSE, sw = Math.sin(phase) * u * .3;
  c.save(); c.lineJoin = 'round'; c.lineCap = 'round';
  const stroke = (w, col, pts) => { c.strokeStyle = col; c.lineWidth = w; c.beginPath(); pts.forEach(([px, py], i) => i ? c.lineTo(px, py) : c.moveTo(px, py)); c.stroke(); };
  // legs
  for (const [s, o] of [[-1, sw], [1, -sw]]) {
    const pts = [[x + s * u * .22, y - u * 1.9], [x + s * u * .25 + o, y - u * .15]];
    stroke(u * .34 + 5, '#1a1a1a', pts); stroke(u * .34, N.pants, pts);
    c.fillStyle = '#1a1a1a'; c.beginPath(); c.arc(x + s * u * .25 + o, y - u * .15, u * .2 + 2.5, 0, 7); c.fill();
    c.fillStyle = '#fff'; c.beginPath(); c.arc(x + s * u * .25 + o, y - u * .15, u * .2, 0, 7); c.fill();
  }
  // body
  c.fillStyle = N.scrubs; c.strokeStyle = '#1a1a1a'; c.lineWidth = 4;
  c.beginPath(); c.roundRect(x - u * .6, y - u * 3.35, u * 1.2, u * 1.6, u * .25); c.fill(); c.stroke();
  c.beginPath(); c.moveTo(x - u * .22, y - u * 3.35); c.lineTo(x, y - u * 3.0); c.lineTo(x + u * .22, y - u * 3.35); c.stroke();
  c.fillStyle = '#fff'; c.fillRect(x + dir * u * .18, y - u * 2.9, u * .3, u * .3);
  c.fillStyle = '#e63946'; c.fillRect(x + dir * u * .18 + u * .11, y - u * 2.86, u * .08, u * .22); c.fillRect(x + dir * u * .18 + u * .04, y - u * 2.79, u * .22, u * .08);
  // arms (one holding a first-aid kit, reaching forward while working)
  const reach = working ? Math.sin(time * 12) * u * .15 : 0;
  const back = [[x - dir * u * .55, y - u * 3.1], [x - dir * u * .75, y - u * 2.4 - sw * .3]];
  const front = [[x + dir * u * .55, y - u * 3.1], [x + dir * (u * .95 + reach), y - u * 2.5]];
  for (const arm of [back, front]) { stroke(u * .3 + 5, '#1a1a1a', arm); stroke(u * .3, N.scrubs, arm); }
  c.fillStyle = N.skin; c.strokeStyle = '#1a1a1a'; c.lineWidth = 2.5;
  for (const arm of [back, front]) { c.beginPath(); c.arc(arm[1][0], arm[1][1], u * .16, 0, 7); c.fill(); c.stroke(); }
  const kx = front[1][0] + dir * u * .1, ky = front[1][1] + u * .15;
  c.fillStyle = '#fff'; c.beginPath(); c.roundRect(kx - u * .28, ky, u * .56, u * .42, 5); c.fill(); c.stroke();
  c.fillStyle = '#e63946'; c.fillRect(kx - u * .04, ky + u * .07, u * .08, u * .28); c.fillRect(kx - u * .14, ky + u * .17, u * .28, u * .08);
  // head
  const hx = x, hy = y - u * 3.95, R = u * .52;
  c.fillStyle = N.hair; c.beginPath(); c.arc(hx - dir * R * .7, hy - R * .6, R * .45, 0, 7); c.fill(); c.stroke(); // bun
  c.fillStyle = N.skin; c.beginPath(); c.arc(hx, hy, R, 0, 7); c.fill(); c.stroke();
  c.fillStyle = N.hair; c.beginPath(); c.arc(hx, hy, R, Math.PI * 1.05, Math.PI * 1.95); c.quadraticCurveTo(hx, hy - R * .4, hx - R, hy - R * .15); c.fill();
  // cap
  c.fillStyle = '#fff'; c.beginPath(); c.moveTo(hx - R * .6, hy - R * .75); c.lineTo(hx + R * .6, hy - R * .75); c.lineTo(hx + R * .45, hy - R * 1.25); c.lineTo(hx - R * .45, hy - R * 1.25); c.closePath(); c.fill(); c.stroke();
  c.fillStyle = '#e63946'; c.fillRect(hx - R * .06, hy - R * 1.18, R * .12, R * .36); c.fillRect(hx - R * .18, hy - R * 1.06, R * .36, R * .12);
  // face
  c.fillStyle = '#1a1a1a';
  for (const s of [-1, 1]) { c.beginPath(); c.arc(hx + s * R * .35 + dir * R * .1, hy, R * .09, 0, 7); c.fill(); }
  c.lineWidth = 3; c.beginPath(); c.arc(hx + dir * R * .1, hy + R * .2, R * .28, .2 * Math.PI, .8 * Math.PI); c.stroke();
  c.fillStyle = 'rgba(255,90,110,.35)'; for (const s of [-1, 1]) { c.beginPath(); c.arc(hx + s * R * .55, hy + R * .25, R * .12, 0, 7); c.fill(); }
  c.restore();
}

// ---------- vehicles: drive across the room and run him over ----------
// len/h in body units, speed in units per step, launch = how high he flies, crush = presses him flat instead
const VEHICLES = {
  bike: { len: 1.7, h: 1.2, speed: .22, dmg: 12, launch: .45, snd: 'engineHi', words: ['VROOM!', 'WHEELIE!'] },
  car: { len: 2.8, h: 1.25, speed: .26, dmg: 18, launch: .5, snd: 'engine', words: ['SPLAT!', 'HIT & RUN!'] },
  icecream: { len: 3.2, h: 1.9, speed: .2, dmg: 18, launch: .45, snd: 'jingle', words: ['ICE SCREAM!', 'BRAIN FREEZE!'] },
  truck: { len: 4, h: 2.1, speed: .22, dmg: 26, launch: .55, snd: 'horn', words: ['HONK!', 'ROADKILL!'] },
  monster: { len: 3.4, h: 2.7, speed: .22, dmg: 30, launch: .7, snd: 'engineLo', words: ['CRUSH!', 'MONSTER JAM!'] },
  roller: { len: 3.2, h: 1.9, speed: .07, dmg: 6, crush: true, snd: 'engineLo', words: ['FLATTENED!', 'PANCAKE!'] },
  rocket: { len: 2.6, h: .95, speed: .6, dmg: 32, launch: 1.1, snd: 'rocket', words: ['LAUNCHED!', 'MACH 5!'] },
  train: { len: 13, h: 2.5, speed: .45, dmg: 45, launch: .8, snd: 'horn', words: ['CHOO CHOO!', 'TRAIN\'D!'] },
};
function driveVehicle(type) {
  const V = VEHICLES[type], [ax] = bodyCenter(arham), dir = ax > W / 2 ? 1 : -1;
  const L = V.len * U;
  const e = { type, V, dir, x: dir > 0 ? -U * .2 : W + U * .2, hit: new Map(), t: 0, wheel: 0 };
  sfx(V.snd); if (type === 'train') setTimeout(() => sfx('horn'), 350);
  e.u = dt => {
    e.t += dt;
    const sp = V.speed * U;
    e.x += dir * sp; e.wheel += sp / (U * .3);
    const back = e.x - dir * L, top = FLOOR - V.h * U;
    if (Math.random() < .5) spawn('smoke', back - dir * 6, FLOOR - U * .3, -dir * rand(.5, 2), rand(-1.2, -.2), rand(.4, .8), rand(6, 12), type === 'rocket' ? '#ffb703' : '#999');
    if (type === 'train' && Math.random() < .6) spawn('smoke', e.x - dir * U * .5, FLOOR - V.h * U * 1.1, -dir * rand(1, 3), rand(-3, -1), rand(.8, 1.4), rand(12, 22), '#555');
    if (type === 'rocket') for (let i = 0; i < 3; i++) spawn('flame', back - dir * rand(4, 20), FLOOR - U * .45 + rand(-8, 8), -dir * rand(4, 9), rand(-1, 1), rand(.15, .3), rand(8, 14), '#ff9f1c');
    for (const B of bodies()) {
      let hitAny = false;
      B.P.forEach((p, i) => {
        const rel = (p.x - e.x) * dir; // 0 at the front bumper, -L at the back
        if (rel > p.r || rel < -L - p.r || p.y < top - p.r) return;
        const key = B.kind + i, last = e.hit.get(key) || -9;
        if (V.crush) { // steamroller: pressed flat and dragged along
          p.y = Math.max(p.y, FLOOR - p.r * .6); p.py = p.y;
          p.x += dir * sp * .9;
          if (e.t - last > .25) { e.hit.set(key, e.t); hurt(B, V.dmg); woundPoint(B, i, V.dmg * 2.2); blood(p.x, p.y, 6, .8); if (Math.random() < .3) gibs(p.x, p.y, 1, .6); hitAny = true; }
          return;
        }
        if (rel > -U * .5) { p.x = e.x + dir * p.r; p.px = p.x - dir * sp * 1.4; } // shoved by the bumper
        else { p.y = Math.min(p.y, top - p.r); } // rolled up over the bonnet
        if (e.t - last > .5) {
          e.hit.set(key, e.t);
          addVel(p, dir * sp * rand(.6, 1.2), -V.launch * U * rand(.6, 1.1));
          hurt(B, V.dmg * rand(.5, 1)); woundPoint(B, i, V.dmg * 1.4);
          hitAny = true;
        }
      });
      if (hitAny) {
        knock(B, 0);
        const [cx, cy] = bodyCenter(B);
        if (!e.announced) { e.announced = true; sfx('crash'); popText(cx, cy - U, pick(V.words), '#ffd84d', 44); shake = Math.max(shake, 22); hitStop = Math.max(hitStop, .09); gibs(cx, cy, 6, 1.4); blood(cx, cy, 24, 1.8); wallSplat(cx, cy - U, 2); }
        else if (Math.random() < .4) sfx('crash');
      }
    }
    return dir > 0 ? back < W + U : back > -U;
  };
  e.d = () => { ctx.save(); ctx.translate(e.x, FLOOR); ctx.scale(dir, 1); drawVehicle(type, V.len * U, V.h * U, e.wheel, e.t); ctx.restore(); };
  ents.push(e);
}
function wheel(x, r, spin, rim = '#bbb') {
  ctx.fillStyle = '#1b1b1b'; ctx.beginPath(); ctx.arc(x, -r, r, 0, 7); ctx.fill(); ctx.stroke();
  ctx.fillStyle = rim; ctx.beginPath(); ctx.arc(x, -r, r * .5, 0, 7); ctx.fill(); ctx.stroke();
  ctx.strokeStyle = '#444'; ctx.lineWidth = 2;
  for (let k = 0; k < 3; k++) { const a = spin + k * 2.09; ctx.beginPath(); ctx.moveTo(x, -r); ctx.lineTo(x + Math.cos(a) * r * .5, -r + Math.sin(a) * r * .5); ctx.stroke(); }
  ctx.strokeStyle = '#111'; ctx.lineWidth = 3;
}
function vbody(x, y, w, h, c, rad = 8) { ctx.fillStyle = c; ctx.beginPath(); ctx.roundRect(x, y, w, h, rad); ctx.fill(); ctx.stroke(); }
function windowPane(x, y, w, h) { ctx.fillStyle = '#9ad1ff'; ctx.beginPath(); ctx.roundRect(x, y, w, h, 4); ctx.fill(); ctx.stroke(); ctx.fillStyle = 'rgba(255,255,255,.5)'; ctx.fillRect(x + w * .15, y + 3, w * .15, h - 6); }
// drawn facing +x with the front bumper at x = 0 and the ground at y = 0
function drawVehicle(type, L, H, spin, t) {
  const u = U;
  ctx.lineWidth = 3; ctx.strokeStyle = '#111'; ctx.lineJoin = 'round';
  if (type === 'bike') {
    wheel(-L * .15, u * .3, spin); wheel(-L * .85, u * .3, spin);
    ctx.strokeStyle = '#ff7b00'; ctx.lineWidth = 6;
    ctx.beginPath(); ctx.moveTo(-L * .85, -u * .3); ctx.lineTo(-L * .5, -u * .6); ctx.lineTo(-L * .15, -u * .3); ctx.moveTo(-L * .5, -u * .6); ctx.lineTo(-L * .25, -u * .85); ctx.stroke();
    ctx.lineWidth = 3; ctx.strokeStyle = '#111';
    vbody(-L * .7, -u * .8, L * .4, u * .22, '#ff7b00', 6);
    // rider
    ctx.strokeStyle = '#223'; ctx.lineWidth = u * .16; ctx.lineCap = 'round';
    ctx.beginPath(); ctx.moveTo(-L * .5, -u * .8); ctx.lineTo(-L * .45, -u * 1.25); ctx.lineTo(-L * .25, -u * .9); ctx.stroke();
    ctx.lineWidth = 3; ctx.strokeStyle = '#111';
    ctx.fillStyle = '#e63946'; ctx.beginPath(); ctx.arc(-L * .42, -u * 1.4, u * .2, 0, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#111'; ctx.fillRect(-L * .38, -u * 1.45, u * .15, u * .08);
  } else if (type === 'car' || type === 'rocket') {
    const c = type === 'car' ? '#e63946' : '#c0c7d1';
    vbody(-L, -H * .75, L, H * .45, c, 12);
    ctx.fillStyle = c; ctx.beginPath(); ctx.moveTo(-L * .75, -H * .74); ctx.lineTo(-L * .62, -H); ctx.lineTo(-L * .3, -H); ctx.lineTo(-L * .2, -H * .74); ctx.closePath(); ctx.fill(); ctx.stroke();
    windowPane(-L * .6, -H * .96, L * .14, H * .2); windowPane(-L * .44, -H * .96, L * .14, H * .2);
    ctx.fillStyle = '#ffe066'; ctx.beginPath(); ctx.arc(-6, -H * .55, 5, 0, 7); ctx.fill(); ctx.stroke();
    if (type === 'rocket') {
      ctx.fillStyle = '#e63946'; ctx.beginPath(); ctx.moveTo(-L, -H * .75); ctx.lineTo(-L * 1.12, -H * 1.15); ctx.lineTo(-L * .85, -H * .75); ctx.fill(); ctx.stroke();
      vbody(-L * 1.08, -H * .65, L * .12, H * .3, '#555', 4);
    }
    wheel(-L * .2, u * .3, spin); wheel(-L * .8, u * .3, spin);
  } else if (type === 'truck' || type === 'icecream') {
    const box = type === 'truck' ? '#f1f1f1' : '#ffc8dd', cab = type === 'truck' ? '#1d4ed8' : '#ffafcc';
    vbody(-L, -H, L * .7, H * .85, box, 6);
    vbody(-L * .3, -H * .75, L * .3, H * .6, cab, 10);
    windowPane(-L * .22, -H * .7, L * .16, H * .22);
    const txt = type === 'truck' ? 'PAIN CO.' : 'ICE SCREAM';
    ctx.save(); ctx.translate(-L * .65, -H * .6); if (ctx.getTransform().a < 0) ctx.scale(-1, 1);
    ctx.fillStyle = type === 'truck' ? '#e63946' : '#7b2cbf'; ctx.font = `900 ${u * .38}px "Trebuchet MS", sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(txt, 0, 0);
    ctx.restore();
    if (type === 'icecream') { // cone on the roof
      ctx.fillStyle = '#d4a373'; ctx.beginPath(); ctx.moveTo(-L * .7, -H); ctx.lineTo(-L * .62, -H - u * .7); ctx.lineTo(-L * .54, -H); ctx.closePath(); ctx.fill(); ctx.stroke();
      ctx.fillStyle = '#ff85a1'; ctx.beginPath(); ctx.arc(-L * .62, -H - u * .75, u * .25, 0, 7); ctx.fill(); ctx.stroke();
    }
    for (const x of [-.12, -.55, -.85]) wheel(L * x, u * .35, spin);
  } else if (type === 'monster') {
    vbody(-L * .9, -H * .78, L * .85, H * .3, '#38b000', 10);
    ctx.fillStyle = '#38b000'; ctx.beginPath(); ctx.moveTo(-L * .7, -H * .77); ctx.lineTo(-L * .6, -H); ctx.lineTo(-L * .3, -H); ctx.lineTo(-L * .22, -H * .77); ctx.closePath(); ctx.fill(); ctx.stroke();
    windowPane(-L * .56, -H * .96, L * .24, H * .14);
    ctx.fillStyle = '#ff7b00'; for (let k = 0; k < 4; k++) { ctx.beginPath(); ctx.moveTo(-L * (.85 - k * .12), -H * .5); ctx.quadraticCurveTo(-L * (.8 - k * .12), -H * .66, -L * (.75 - k * .12), -H * .5); ctx.fill(); }
    ctx.strokeStyle = '#555'; ctx.lineWidth = 6; for (const x of [-.25, -.75]) { ctx.beginPath(); ctx.moveTo(L * x, -u * .8); ctx.lineTo(L * x, -H * .5); ctx.stroke(); }
    ctx.lineWidth = 3; ctx.strokeStyle = '#111';
    wheel(-L * .25, u * .8, spin, '#ffd60a'); wheel(-L * .75, u * .8, spin, '#ffd60a');
  } else if (type === 'roller') {
    vbody(-L, -H * .75, L * .6, H * .45, '#ffd60a', 8);
    vbody(-L * .85, -H, L * .35, H * .3, '#ffd60a', 6); windowPane(-L * .78, -H * .95, L * .2, H * .18);
    ctx.fillStyle = '#555'; ctx.fillRect(-L * .42, -H * .55, L * .18, H * .1); ctx.strokeRect(-L * .42, -H * .55, L * .18, H * .1);
    ctx.fillStyle = '#7a7a7a'; ctx.beginPath(); ctx.arc(-L * .2, -u * .65, u * .65, 0, 7); ctx.fill(); ctx.stroke(); // drum
    ctx.strokeStyle = '#555'; ctx.lineWidth = 2; for (let k = 0; k < 6; k++) { const a = spin * .4 + k; ctx.beginPath(); ctx.moveTo(-L * .2 + Math.cos(a) * u * .2, -u * .65 + Math.sin(a) * u * .2); ctx.lineTo(-L * .2 + Math.cos(a) * u * .6, -u * .65 + Math.sin(a) * u * .6); ctx.stroke(); }
    ctx.strokeStyle = '#111'; ctx.lineWidth = 3;
    wheel(-L * .85, u * .45, spin);
  } else if (type === 'train') {
    // locomotive at the front, one boxcar behind
    vbody(-L * .42, -H * .8, L * .42, H * .55, '#222', 10);
    ctx.fillStyle = '#e63946'; ctx.fillRect(-L * .42, -H * .38, L * .42, H * .08);
    vbody(-L * .42, -H, L * .12, H * .45, '#333', 6); windowPane(-L * .4, -H * .95, L * .08, H * .18);
    vbody(-L * .14, -H * 1.1, u * .35, H * .35, '#111', 4); // chimney
    ctx.fillStyle = '#888'; ctx.beginPath(); ctx.moveTo(0, -H * .3); ctx.lineTo(u * .5, 0); ctx.lineTo(0, 0); ctx.closePath(); ctx.fill(); ctx.stroke(); // cow catcher
    ctx.fillStyle = '#ffe066'; ctx.beginPath(); ctx.arc(-u * .1, -H * .65, u * .15, 0, 7); ctx.fill(); ctx.stroke();
    vbody(-L, -H * .95, L * .55, H * .75, '#8d5524', 6);
    ctx.strokeStyle = '#5a3a1a'; for (let k = 1; k < 8; k++) { ctx.beginPath(); ctx.moveTo(-L + k * L * .55 / 8, -H * .92); ctx.lineTo(-L + k * L * .55 / 8, -H * .25); ctx.stroke(); }
    ctx.strokeStyle = '#111';
    for (const x of [-.06, -.16, -.28, -.38, -.52, -.62, -.85, -.95]) wheel(L * x, u * .32, spin, '#e63946');
  }
}

// ---------- orbital strike cannon ----------
function orbitalStrike(tx) {
  const e = { x: tx, t: 0, state: 'aim' };
  sfx('charge');
  e.u = dt => {
    e.t += dt;
    if (e.state === 'aim') {
      if (e.t > 2) { e.state = 'fire'; e.t = 0; sfx('beam'); flash = 1; flashColor = '#c8f7ff'; }
    } else if (e.state === 'fire') {
      shake = Math.max(shake, 22); hurtFx = Math.max(hurtFx, .2);
      const w = U * .9;
      for (const B of bodies()) {
        let inBeam = false;
        B.P.forEach((p, i) => {
          if (Math.abs(p.x - e.x) > w + p.r) return;
          inBeam = true;
          addVel(p, (p.x - e.x) * .02 + rand(-1, 1), rand(-2, 1.5));
          if (Math.random() < .2) woundPoint(B, i, 14);
        });
        if (inBeam) {
          hurt(B, dt * 90, 'zap'); knock(B, 0); B.char = Math.min(1, B.char + dt * .5);
          if (Math.random() < .35) { const [cx, cy] = bodyCenter(B); gibs(cx, cy, 1, 1.3); blood(cx, cy, 4, 1.4); }
        }
      }
      for (let k = 0; k < 4; k++) spawn('spark', e.x + rand(-w, w), FLOOR - rand(0, 10), rand(-9, 9), rand(-12, -2), rand(.2, .5), rand(2, 4), pick(['#fff', '#72efdd', '#c8f7ff']));
      if (Math.random() < .5) spawn('smoke', e.x + rand(-w, w) * 1.5, FLOOR - rand(0, U), rand(-3, 3), rand(-3, -1), rand(.6, 1.2), rand(15, 30), '#444');
      if (e.t > 1.8) {
        explode(e.x, FLOOR - 10, U * 5, U * 1.4, { fire: true, text: 'ORBITAL STRIKE!' });
        whiteT = Math.max(whiteT, 1.2); shake = 45;
        wallMarks.push({ x: e.x, y: FLOOR - 5, r: U * 1.6, life: 40 });
        return false;
      }
    }
    return true;
  ...
  e.d = () => {
    if (e.state === 'aim') {
      const blink = (e.t * 8 | 0) % 2, r = U * (1.6 - e.t * .5);
      ctx.strokeStyle = blink ? '#ff2b2b' : '#fff'; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.arc(e.x, FLOOR - U * .4, Math.max(8, r), 0, 7); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(e.x - U * 2, FLOOR - U * .4); ctx.lineTo(e.x + U * 2, FLOOR - U * .4); ctx.moveTo(e.x, FLOOR - U * 2.2); ctx.lineTo(e.x, FLOOR + 10); ctx.stroke();
      ctx.strokeStyle = `rgba(255,40,40,${.3 + .4 * Math.random()})`; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(e.x, 0); ctx.lineTo(e.x, FLOOR); ctx.stroke();
      ctx.font = '900 20px "Trebuchet MS", sans-serif'; ctx.textAlign = 'center'; ctx.lineWidth = 5; ctx.strokeStyle = '#000'; ctx.fillStyle = '#ff2b2b';
      const t = `ORBITAL LOCK ${Math.max(0, 2 - e.t).toFixed(1)}s`; ctx.strokeText(t, e.x, FLOOR - U * 2.5); ctx.fillText(t, e.x, FLOOR - U * 2.5);
    } else {
      const w = U * .9 * (1 + Math.sin(e.t * 40) * .08);
      const g = ctx.createLinearGradient(e.x - w * 2, 0, e.x + w * 2, 0);
      g.addColorStop(0, 'rgba(114,239,221,0)'); g.addColorStop(.3, 'rgba(114,239,221,.6)'); g.addColorStop(.45, '#fff'); g.addColorStop(.55, '#fff'); g.addColorStop(.7, 'rgba(114,239,221,.6)'); g.addColorStop(1, 'rgba(114,239,221,0)');
      ctx.fillStyle = g; ctx.fillRect(e.x - w * 2, -60, w * 4, FLOOR + 60);
      ctx.fillStyle = 'rgba(255,255,255,.7)'; ctx.beginPath(); ctx.ellipse(e.x, FLOOR, w * 2.2, U * .35, 0, 0, 7); ctx.fill();
    }
  };
  ents.push(e);
}

// ---------- wacky gadgets (always-open shop tab) ----------
function nearBodies(x, y, r, fn) {
  for (const B of bodies()) B.P.forEach((p, i) => { const d = Math.hypot(p.x - x, p.y - y); if (d < r) fn(B, p, i, d); });
}

// Giant Boot: stomps down from the ceiling onto the tapped spot
function giantBoot(tx) {
  const e = { x: clamp(tx, U, W - U), y: -U * 3, vy: 0, state: 'drop', t: 0 };
  sfx('whoosh');
  e.u = dt => {
    if (e.state === 'drop') {
      e.vy += U * .05; e.y += e.vy;
      if (e.y >= FLOOR - U * 1.6) {
        e.y = FLOOR - U * 1.6; e.state = 'hold'; e.t = 0;
        sfx('crash'); sfx('splat'); shake = Math.max(shake, 30); hitStop = Math.max(hitStop, .1);
        nearBodies(e.x, FLOOR, U * 1.4, (B, p, i, d) => {
          p.y = Math.max(p.y, FLOOR - p.r * .6); p.py = p.y;
          addVel(p, (p.x - e.x) * .04, 0);
          hurt(B, 22 * (1 - d / (U * 1.4))); woundPoint(B, i, 30); knock(B, 0);
        });
        const [cx, cy] = bodyCenter(arham); popText(cx, cy - U, 'STOMP!', '#ffd84d', 46);
        blood(e.x, FLOOR - 6, 24, 1.6); dust(e.x, 16);
      }
    } else { e.t += dt; if (e.t > .6) { e.y -= U * .12; if (e.y < -U * 3) return false; } }
    return true;
  };
  e.d = () => {
    const x = e.x, y = e.y, u = U;
    ctx.save(); ctx.translate(x, y); ctx.lineWidth = 4; ctx.strokeStyle = '#111';
    ctx.fillStyle = '#5b3a1a'; ctx.beginPath(); ctx.roundRect(-u * .55, -u * 1.6, u * 1.0, u * 1.4, 12); ctx.fill(); ctx.stroke(); // leg
    ctx.fillStyle = '#6b4423'; ctx.beginPath(); ctx.roundRect(-u * .8, -u * .4, u * 1.9, u * .55, [10, 20, 14, 14]); ctx.fill(); ctx.stroke(); // foot
    ctx.fillStyle = '#3a2412'; ctx.beginPath(); ctx.roundRect(-u * .85, u * .05, u * 2.0, u * .2, 6); ctx.fill(); ctx.stroke(); // sole
    ctx.strokeStyle = '#caa472'; ctx.lineWidth = 3; for (let k = 0; k < 3; k++) { ctx.beginPath(); ctx.moveTo(-u * .4, -u * 1.4 + k * u * .35); ctx.lineTo(u * .3, -u * 1.4 + k * u * .35); ctx.stroke(); }
    ctx.restore();
  };
  ents.push(e);
}

// Tornado: a spinning funnel that sweeps in, sucks him up and spits him out
function tornado() {
  const dir = bodyCenter(arham)[0] > W / 2 ? -1 : 1;
  const e = { x: dir > 0 ? -U : W + U, dir, t: 0, life: 6, debris: [] };
  sfx('fwoosh');
  e.u = dt => {
    e.t += dt; e.life -= dt;
    e.x += dir * U * .04 + Math.sin(e.t * 2) * U * .015;
    e.x = clamp(e.x, U, W - U);
    nearBodies(e.x, FLOOR - U * 1.5, U * 2.2, (B, p, i, d) => {
      const ang = e.t * 12 + i, lift = clamp(1 - (FLOOR - p.y) / (U * 4), .2, 1);
      addVel(p, (e.x - p.x) * .04 + Math.cos(ang) * U * .04, -U * .05 * lift + Math.sin(ang) * U * .02);
      knock(B, 0);
      if (Math.random() < dt * 6) { hurt(B, 2); }
    });
    if (Math.random() < .4) e.debris.push({ a: rand(0, 7), r: rand(U, U * 2), y: rand(-U * 3.5, 0), s: rand(3, 6) });
    for (const d of e.debris) { d.a += dt * 8; d.y += dt * 4; }
    e.debris = e.debris.filter(d => d.y < U);
    if (Math.random() < .3) sfx('fwoosh');
    if (e.life <= 0) { nearBodies(e.x, FLOOR - U * 2, U * 2.5, (B, p) => addVel(p, e.dir * U * .3, -U * .2)); return false; }
    return true;
  };
  e.d = () => {
    const x = e.x, u = U;
    ctx.save(); ctx.globalAlpha = clamp(Math.min(e.t * 2, e.life), 0, 1) * .85;
    for (let k = 0; k < 9; k++) {
      const yy = FLOOR - k * u * .45, wobble = Math.sin(e.t * 8 + k) * u * .12;
      const w = u * (.25 + k * .28);
      ctx.fillStyle = k % 2 ? 'rgba(150,150,160,.5)' : 'rgba(190,190,200,.4)';
      ctx.beginPath(); ctx.ellipse(x + wobble, yy, w, u * .22, 0, 0, 7); ctx.fill();
    }
    ctx.globalAlpha = clamp(Math.min(e.t * 2, e.life), 0, 1);
    for (const d of e.debris) { ctx.fillStyle = '#8a6d3b'; ctx.beginPath(); ctx.arc(x + Math.cos(d.a) * d.r, FLOOR - u * 2 + d.y, d.s, 0, 7); ctx.fill(); }
    ctx.restore();
  };
  ents.push(e);
}

// Magnet: a giant horseshoe magnet lowers from the ceiling and yanks him up by the metal in his bones
function magnet(tx) {
  const e = { x: clamp(tx, U, W - U), y: -U * 2, state: 'down', t: 0 };
  sfx('engineHi');
  e.u = dt => {
    e.t += dt;
    if (e.state === 'down') { e.y += U * .05; if (e.y > U * 1.2) e.state = 'pull'; }
    else if (e.state === 'pull') {
      const my = e.y + U * 1.1;
      nearBodies(e.x, my, U * 6, (B, p, i, d) => {
        const f = clamp(U * 4 / (d + U), .2, 2) * U * .03;
        addVel(p, (e.x - p.x) / (d + 1) * f * 10, (my - p.y) / (d + 1) * f * 10);
        knock(B, 0);
        if (p.y < my + U && Math.random() < dt * 3) hurt(B, 1);
      });
      if (Math.random() < .5) spawn('spark', e.x + rand(-U, U), my + rand(0, U * 2), 0, -3, .3, 2, '#72efdd');
      if (e.t > 4) e.state = 'up';
    } else { e.y -= U * .06; if (e.y < -U * 2) { nearBodies(e.x, e.y + U * 4, U * 5, (B, p) => addVel(p, 0, U * .3)); return false; } }
    return true;
  };
  e.d = () => {
    const x = e.x, y = e.y, u = U;
    ctx.strokeStyle = '#444'; ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(x, -60); ctx.lineTo(x, y - u * .5); ctx.stroke();
    ctx.save(); ctx.translate(x, y); ctx.lineWidth = 4; ctx.strokeStyle = '#111'; ctx.lineJoin = 'round';
    ctx.fillStyle = '#c1121f';
    ctx.beginPath();
    ctx.moveTo(-u * .7, 0); ctx.lineTo(-u * .7, u); ctx.lineTo(-u * .3, u); ctx.lineTo(-u * .3, u * .3);
    ctx.arc(0, u * .3, u * .3, Math.PI, 0, true); ctx.lineTo(u * .3, u); ctx.lineTo(u * .7, u); ctx.lineTo(u * .7, 0);
    ctx.arc(0, 0, u * .7, 0, Math.PI); ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#dfe6ec'; ctx.fillRect(-u * .7, u * .95, u * .4, u * .35); ctx.strokeRect(-u * .7, u * .95, u * .4, u * .35);
    ctx.fillRect(u * .3, u * .95, u * .4, u * .35); ctx.strokeRect(u * .3, u * .95, u * .4, u * .35);
    ctx.restore();
  };
  ents.push(e);
}

// Boxing Glove: a spring-loaded glove rockets in from the side and cross-checks him across the room
function boxingGlove() {
  const [ax] = bodyCenter(arham), dir = ax > W / 2 ? 1 : -1;
  const e = { x: dir > 0 ? -U : W + U, y: FLOOR - U * 2.4, dir, t: 0, hit: false, springLen: 0 };
  sfx('whoosh');
  e.u = dt => {
    e.t += dt;
    if (!e.hit) {
      e.x += dir * U * .5; e.springLen = Math.min((e.dir > 0 ? e.x : W - e.x), U * 5);
      nearBodies(e.x + dir * U * .5, e.y, U * .9, (B, p, i) => {
        e.hit = true; sfx('punch'); sfx('crash'); shake = Math.max(shake, 20); hitStop = Math.max(hitStop, .1);
        for (const q of B.P) addVel(q, dir * U * .55, -U * .25);
        hurt(B, 30); woundPoint(B, i, 20); knock(B, 0);
        const [cx, cy] = bodyCenter(B); popText(cx, cy - U, pick(['KA-POW!', 'BONK!', 'YEET!']), '#ff5d73', 48);
        parts.push({ type: 'burst', x: e.x + dir * U * .6, y: e.y, vx: 0, vy: 0, life: .25, max: .25, size: U * 1.5, color: '#fff' });
      });
      if ((dir > 0 && e.x > W + U) || (dir < 0 && e.x < -U)) return false;
    } else { e.x -= dir * U * .35; if ((dir > 0 && e.x < -U) || (dir < 0 && e.x > W + U)) return false; }
    return true;
  };
  e.d = () => {
    const u = U, y = e.y;
    ctx.strokeStyle = '#d4d4d4'; ctx.lineWidth = 6; ctx.lineCap = 'round'; // spring/arm
    const base = e.dir > 0 ? 0 : W;
    ctx.beginPath(); ctx.moveTo(base, y);
    const coils = 8, span = e.x - e.dir * u * .5 - base;
    for (let k = 0; k <= coils; k++) ctx.lineTo(base + span * k / coils, y + (k % 2 ? -u * .2 : u * .2));
    ctx.stroke();
    ctx.save(); ctx.translate(e.x, y); ctx.scale(e.dir, 1); ctx.lineWidth = 4; ctx.strokeStyle = '#111';
    ctx.fillStyle = '#d1121f'; ctx.beginPath(); ctx.roundRect(-u * .5, -u * .5, u * .7, u, 10); ctx.fill(); ctx.stroke();
    ctx.beginPath(); ctx.arc(u * .15, -u * .1, u * .5, -1.3, 1.3); ctx.arc(u * .35, u * .25, u * .28, 1.3, -.4, true); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#fff'; ctx.fillRect(-u * .5, u * .1, u * .7, u * .14);
    ctx.restore();
  };
  ents.push(e);
}

// Falling Whale: a startled cartoon whale plummets from the sky and squashes him
function whaleDrop(tx) {
  const e = { x: clamp(tx, U, W - U), y: -U * 3, vy: 0, landed: 0 };
  sfx('engineLo');
  e.u = dt => {
    if (e.landed) { e.landed -= dt; if (e.landed <= 0) return false; return true; }
    e.vy += U * .04; e.y += e.vy;
    if (e.y >= FLOOR - U * 1.1) {
      e.y = FLOOR - U * 1.1; e.landed = 1.6;
      sfx('splat'); sfx('crash'); shake = Math.max(shake, 34); hitStop = Math.max(hitStop, .12);
      nearBodies(e.x, FLOOR, U * 2.6, (B, p, i, d) => {
        p.y = Math.max(p.y, FLOOR - p.r * .5); p.py = p.y; addVel(p, (p.x - e.x) * .05, 0);
        hurt(B, 30 * (1 - d / (U * 2.6))); woundPoint(B, i, 26); knock(B, 0);
      });
      const [cx, cy] = bodyCenter(arham); popText(cx, cy - U * 1.2, 'SQUISH!', '#4cc9f0', 50);
      blood(e.x, FLOOR - 6, 30, 1.8); dust(e.x, 20);
    }
    return true;
  };
  e.d = () => {
    const x = e.x, y = e.y, u = U, squash = e.landed ? .78 : 1;
    ctx.save(); ctx.translate(x, y); ctx.scale(1, squash); ctx.lineWidth = 4; ctx.strokeStyle = '#111'; ctx.lineJoin = 'round';
    ctx.fillStyle = '#4a90d9'; ctx.beginPath(); ctx.ellipse(0, 0, u * 1.7, u * 1.05, 0, 0, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#bcd8f0'; ctx.beginPath(); ctx.ellipse(-u * .3, u * .35, u * 1.2, u * .55, 0, 0, 7); ctx.fill();
    ctx.fillStyle = '#4a90d9'; ctx.beginPath(); ctx.moveTo(u * 1.5, -u * .1); ctx.lineTo(u * 2.2, -u * .7); ctx.lineTo(u * 2.15, u * .1); ctx.lineTo(u * 1.5, u * .3); ctx.closePath(); ctx.fill(); ctx.stroke(); // tail
    ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(-u * 1.05, -u * .25, u * .2, 0, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#111'; ctx.beginPath(); ctx.arc(-u * 1.12, -u * .25, u * .09, 0, 7); ctx.fill();
    ctx.strokeStyle = '#2e5f92'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-u * 1.3, u * .1); ctx.quadraticCurveTo(-u * 1.5, u * .3, -u * 1.3, u * .45); ctx.stroke(); // worried mouth
    if (!e.landed) { ctx.strokeStyle = '#9ad1ff'; ctx.lineWidth = 3; for (const s of [-1, 1]) { ctx.beginPath(); ctx.moveTo(-u * .4, -u * .9); ctx.lineTo(-u * .4 + s * u * .3, -u * 1.4); ctx.stroke(); } } // spout
    ctx.restore();
  };
  ents.push(e);
}

// Fan: a giant desk fan blows him and everything into the far wall
function bigFan() {
  const [ax] = bodyCenter(arham), dir = ax > W / 2 ? 1 : -1;
  const e = { x: dir > 0 ? U * .8 : W - U * .8, dir, t: 0, life: 5, blade: 0 };
  sfx('engine');
  e.u = dt => {
    e.t += dt; e.life -= dt; e.blade += dt * 40;
    nearBodies(e.x, FLOOR - U * 1.6, U * 12, (B, p, i, d) => {
      if ((p.x - e.x) * dir < 0) return;
      addVel(p, dir * U * .08 * clamp(2 - d / (U * 6), .2, 2), -U * .01 + Math.sin(e.t * 20 + i) * U * .01);
      knock(B, 0);
    });
    for (const q of props) if ((q.x - e.x) * dir > 0 && Math.abs(q.y - (FLOOR - U * 1.6)) < U * 3) addVel(q, dir * U * .05, 0);
    if (Math.random() < .3) sfx('fwoosh');
    if (e.life <= 0) return false;
    return true;
  };
  e.d = () => {
    const x = e.x, y = FLOOR - U * 1.6, u = U;
    ctx.strokeStyle = '#333'; ctx.lineWidth = 6; ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x, FLOOR); ctx.stroke(); // stand
    ctx.fillStyle = '#555'; ctx.beginPath(); ctx.ellipse(x, FLOOR, u * .5, u * .15, 0, 0, 7); ctx.fill();
    ctx.save(); ctx.translate(x, y); ctx.scale(e.dir, 1);
    ctx.fillStyle = '#8a8f98'; ctx.strokeStyle = '#111'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, u * 1.05, 0, 7); ctx.fill(); ctx.stroke(); // cage
    ctx.save(); ctx.rotate(e.blade);
    ctx.fillStyle = '#c8ccd2'; for (let k = 0; k < 4; k++) { ctx.rotate(Math.PI / 2); ctx.beginPath(); ctx.ellipse(u * .45, 0, u * .5, u * .28, 0, 0, 7); ctx.fill(); ctx.stroke(); }
    ctx.restore();
    ctx.fillStyle = '#444'; ctx.beginPath(); ctx.arc(0, 0, u * .2, 0, 7); ctx.fill(); ctx.stroke();
    ctx.strokeStyle = 'rgba(255,255,255,.25)'; ctx.lineWidth = 4;
    for (let k = 0; k < 4; k++) { const yy = -u * .6 + k * u * .4; ctx.beginPath(); ctx.moveTo(u * 1.2, yy); ctx.lineTo(u * 3.5, yy + Math.sin(e.t * 10 + k) * u * .2); ctx.stroke(); }
    ctx.restore();
  };
  ents.push(e);
}
...
function stepBody(B, dt, G) {
  ...
  if (B.bleed > .02 && Math.random() < B.bleed * .6) {
    ...
    spawnBlood(x, y, rand(-.4, .4), rand(0, 1), rand(1.5, 3));
  }
  ...
  if (B.kind === 'boodie') {
    ...
    if (!canStand(B)) B.stand = 0;
    else if (!(grab && grab.B === B) && gravDown() && B.sinceHurt > 1.3 && B.pain < 70 && B.onFire <= 0 && B.zapped <= 0) B.stand = Math.min(1, B.stand + dt * .6);
    if (wasDown && B.stand >= .5 && Math.random() < .5) say(B, 'up');
    if (gravDown() && B.stand > .01) {
  ...
  if (B.targets && B.stand > .01) {
    ...
    const limp = new Set();
    [[2, [6, 8]], [3, [8]], [4, [7, 9]], [5, [9]]].forEach(([f, pts]) => { if (B.broken[f]) pts.forEach(i => limp.add(i)); });
    for (let i = 0; i < 14; i++) {
      if (!B.attached[i] || limp.has(i)) continue; // severed or broken parts dangle
...
function solveBody(B, it) {
  ...
  for (let i = 0; i < 14; i++) { const pin = B.pinned[i]; if (pin) { const p = P[i]; p.x = pin.x; p.y = pin.y; p.px = pin.x; p.py = pin.y; } }
...
function step(dt) {
  ...
  for (const q of props) {
    ...
    q.px = q.x; q.py = q.y; q.x += vx * .998 + gvx(); q.y += vy * .998 + gvy();
    q.hitCd -= dt;
  ...
  for (const B of bs) for (const q of props) for (let i = 0; i < 14; i++) {
    ...
    if (rel > U * .12 && q.hitCd <= 0) {
      ...
      if (h) { addDecal(B, h.f, hx, hy, 'bruise', 0, clamp(q.r / U * 2.5, .8, 2)); wound(B, h.f, Math.min(dmg * 1.2, 90)); }
      if (dmg > 15) gibs(p.x, p.y, Math.min(8, Math.round(dmg / 8)), 1);
    }
  }
  // prop vs prop
  ...
  for (let i = parts.length - 1; i >= 0; i--) {
    ...
    q.life -= dt;
    q.x += q.vx; q.y += q.vy;
    if (q.type === 'drop' || q.type === 'spark' || q.type === 'shell' || q.type === 'tooth' || q.type === 'gib') { q.vx += gvx() * 1.1; q.vy += gvy() * 1.1; }
    if (q.type === 'gib') updateGib(q);
    if (q.type === 'flame') { q.vy -= .05; q.size *= .985; }
    if (q.type === 'smoke') { q.size *= 1.01; q.vx *= .98; }
    if (q.type === 'drop' && q.y > FLOOR) {
      if (q.blood) { addStain(q.x, q.size * rand(1.5, 3)); if (Math.random() < .05) sfx('drip'); }
      else { floorStains.push({ x: q.x, y: FLOOR + rand(2, 10), r: q.size * rand(1.5, 3.2), life: 35, c: q.color }); if (floorStains.length > 140) floorStains.shift(); }
      q.life = 0;
    }
...
function collideWalls(B, p, detect) {
  ...
  if (p.y + p.r > FLOOR) {
    ...
    if (detect && vy > u * .22 && p.hitCd <= 0) {
      ...
      const dmg = (vy / u - .2) * 14;
      sfx('thud', vy / u * 1.5);
      if (dmg > 3) { hurt(B, dmg); if (dmg > 8) woundPoint(B, B.P.indexOf(p), dmg * .7); shake = Math.max(shake, dmg * .6); sparks(p.x, FLOOR, 5, '#fff'); knock(B, 0); dust(p.x, 8); if (dmg > 6) { blood(p.x, FLOOR - 5, 6, 1); ring(p.x, FLOOR - 4, u * .8); } }
    }
    p.y = FLOOR - p.r;
    p.py = p.y + vy * .25;
...
function draw() {
  ...
  for (const s of floorStains) {
    ...
    const ry = Math.min(s.r * .3, 9);
    ctx.fillStyle = s.c || '#6d000d';
    ctx.beginPath(); ctx.ellipse(s.x, s.y, s.r, ry, 0, 0, 7); ctx.fill();
    if (s.blood) { ctx.fillStyle = '#9b0a1c'; ctx.beginPath(); ctx.ellipse(s.x - s.r * .1, s.y - ry * .1, s.r * .75, ry * .65, 0, 0, 7); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,.18)'; ctx.beginPath(); ctx.ellipse(s.x - s.r * .3, s.y - ry * .3, s.r * .25, ry * .18, 0, 0, 7); ctx.fill(); }
  }
  ctx.globalAlpha = 1;
  ...
  drawSpikes();
  for (const B of bodies()) for (let i = 0; i < 14; i++) if (B.pinned[i]) drawPin(B.P[i]);
  for (const B of bodies()) {
  ...
  drawComboHud();
  if (!gravDown() || gravFxT > 0) {
    const d = GRAV_DIRS[gravIdx];
    ctx.globalAlpha = gravDown() ? clamp(gravFxT, 0, 1) : 1;
    ctx.font = '900 20px "Trebuchet MS", sans-serif'; ctx.textAlign = 'center'; ctx.lineWidth = 5; ctx.strokeStyle = '#000'; ctx.fillStyle = '#e0aaff';
    const t = `GRAVITY ${d[3]} ${d[2]}`; ctx.strokeText(t, W / 2, 112); ctx.fillText(t, W / 2, 112); ctx.globalAlpha = 1;
  }
...
// ---------- spike traps ----------
// placed anywhere; a cluster of steel spikes. Bodies driven onto them are impaled.
function placeSpikes(x, y) {
  y = Math.min(y, FLOOR - 2);
  spikes.push({ x, y, hit: new Map(), t: 0 });
  if (spikes.length > 30) spikes.shift();
  sfx('pin'); ring(x, y, U * .5, '#c8ccd2');
}
function stepSpikes(dt) {
  for (const s of spikes) {
    s.t += dt;
    for (const B of bodies()) B.P.forEach((pt, i) => {
      if (Math.abs(pt.x - s.x) > U * .8 || Math.abs(pt.y - s.y) > U * .5) return;
      const key = B.kind + i, last = s.hit.get(key) || -9, speed = Math.hypot(pt.x - pt.px, pt.y - pt.py);
      // push the point up out of the spikes, so he sits impaled on top
      if (pt.y > s.y - U * .35) { pt.y = s.y - U * .35; pt.py = pt.y; }
      if (s.t - last < .35) return;
      s.hit.set(key, s.t);
      const dmg = 6 + speed / U * 20;
      hurt(B, dmg); woundPoint(B, i, dmg * 1.8); knock(B, 0);
      blood(pt.x, pt.y, speed > U * .3 ? 14 : 5, 1 + speed / U);
      if (speed > U * .35) { gibs(pt.x, pt.y, 2, 1); sfx('squelch'); popText(pt.x, pt.y - 14, 'SHINK!', '#c8ccd2', 30); }
      else sfx('splat');
    });
  }
}
function drawSpikes() {
  for (const s of spikes) {
    const u = U, n = 5, w = u * 1.3, x0 = s.x - w / 2;
    ctx.fillStyle = '#3a3f47'; ctx.strokeStyle = '#111'; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.roundRect(x0, s.y, w, u * .22, 4); ctx.fill(); ctx.stroke(); // base plate
    for (let k = 0; k < n; k++) {
      const cx = x0 + w * (k + .5) / n, h = u * (.5 + (k % 2) * .12);
      const g = ctx.createLinearGradient(cx, s.y - h, cx, s.y);
      g.addColorStop(0, '#eef2f6'); g.addColorStop(.5, '#aab2bd'); g.addColorStop(1, '#5b626c');
      ctx.fillStyle = g; ctx.beginPath(); ctx.moveTo(cx - u * .1, s.y + 2); ctx.lineTo(cx, s.y - h); ctx.lineTo(cx + u * .1, s.y + 2); ctx.closePath(); ctx.fill(); ctx.stroke();
    }
  }
}
function drawPin(p) {
  ctx.save(); ctx.translate(p.x, p.y);
  ctx.fillStyle = 'rgba(0,0,0,.3)'; ctx.beginPath(); ctx.ellipse(2, 2, 9, 9, 0, 0, 7); ctx.fill();
  ctx.strokeStyle = '#1a1a1a'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, 13); ctx.stroke();
  ctx.fillStyle = '#e63946'; ctx.strokeStyle = '#1a1a1a'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.arc(0, -4, 8, 0, 7); ctx.fill(); ctx.stroke();
  ctx.fillStyle = 'rgba(255,255,255,.6)'; ctx.beginPath(); ctx.arc(-3, -6, 3, 0, 7); ctx.fill();
  ctx.restore();
}
  ...
  for (const d of B.decals) {
    ...
    switch (d.kind) {
      ...
      case 'blood':
        ...
        break;
      case 'bandage':
        ctx.fillStyle = '#f7f4ec'; ctx.strokeStyle = 'rgba(0,0,0,.35)'; ctx.lineWidth = 1.5 * s;
        ctx.beginPath(); ctx.roundRect(-13 * s, -9 * s, 26 * s, 18 * s, 3 * s); ctx.fill(); ctx.stroke();
        ctx.strokeStyle = 'rgba(0,0,0,.12)'; for (const y of [-4, 0, 4]) { ctx.beginPath(); ctx.moveTo(-12 * s, y * s); ctx.lineTo(12 * s, y * s); ctx.stroke(); }
        ctx.fillStyle = 'rgba(190,20,35,.45)'; ctx.beginPath(); ctx.arc(2 * s, 0, 4 * s, 0, 7); ctx.fill();
        break;
      case 'soot':
...
function drawProp(q) {
  ...
  if (q.type === 'ball') {
    ...
    drawBoneShape(q.len || U * .7, r * .8);
    ctx.fillStyle = '#a4161a'; for (const sx of [-1, 1]) { ctx.beginPath(); ctx.arc(sx * (q.len || U * .7) / 2, 0, r * .45, 0, 7); ctx.fill(); }
...
const PHASES = [
  ...
  { name: 'Vehicles', desc: 'Park directly on him. Always open.', always: true, tab: '🚗' },
  { name: 'Wacky', desc: 'Cartoon physics gone wrong. Always open.', always: true, tab: '🤪' },
];
...
const SHOPKEEPER = {
  ...
  hats: ['cap', 'fire', 'army', 'space', 'clown', 'cap', 'clown'], // one per shop phase: cap | fire | army | space | clown | none
  lines: {
...
function renderShop() {
  ...
  PHASES.forEach((ph, i) => {
    const n = i + 1, locked = n > phasesUnlocked && !ph.always, b = document.createElement('button');
    b.className = 'ptab' + (n === shopPhase ? ' on' : '') + (locked ? ' locked' : '');
    b.textContent = locked ? `🔒${n}` : ph.tab || n; b.title = locked ? 'Fill the pain meter to unlock' : ph.name;
...
function buildTools() {
  ...
  nurse.className = 'tool nursebtn';
  nurse.innerHTML = '<div class="ic">🏥</div><div>Nurse</div>';
  nurse.addEventListener('click', () => { initAudio(); openNurse(true); });
  box.appendChild(nurse);
...
function refreshAffordable() {
  const any = TOOLS.some(t => (t.phase <= phasesUnlocked || PHASES[t.phase - 1].always) && !unlocked.has(t.id) && canAfford(t.price));
...
$('nurseSurgery').onclick = () => { $('nurseSay').textContent = pick(NURSE.lines.surgery); sendNurse('surgery'); };
$('nurseName').textContent = NURSE.name;
...
// global restart combos (Mac + PC): Alt/Option+Shift+R, or tap R three times fast
let rTaps = [];
function openRestart() { initAudio(); $('newGameBox').hidden = false; }
window.addEventListener('keydown', e => {
  if ((e.altKey || e.metaKey) && e.shiftKey && e.code === 'KeyR') { e.preventDefault(); openRestart(); return; }
  const t = (document.activeElement || {}).tagName;
  if (t === 'INPUT' || t === 'TEXTAREA') return; // don't hijack the name/password box for bare taps
  if (e.code === 'KeyR' && !e.altKey && !e.metaKey && !e.ctrlKey) {
    const now = performance.now();
    rTaps = rTaps.filter(x => now - x < 800); rTaps.push(now);
    if (rTaps.length >= 3) { rTaps = []; openRestart(); }
  }
}, true);
...
// password screen: the game stays blank until the developer password is entered
let gateOpen = false;
$('gateForm').onsubmit = e => {
  e.preventDefault();
  if ($('gateField').value === VAULT_PASSWORD) {
    gateOpen = true; $('gate').classList.add('open'); initAudio(); $('gateField').blur();
    ...
    sfx('deny'); $('gateMsg').textContent = 'Wrong password.';
...
function loop(now) {
  ...
  acc += real; last = now;
  const DT = 1 / 60;
  if (!gateOpen) { acc = 0; requestAnimationFrame(loop); return; }
  if (ASC.active) { ASC.frame(Math.max(1 / 60, Math.min(.05, real))); requestAnimationFrame(loop); return; }
