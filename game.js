...
function stepBody(B, dt, G) {
  ...
  if (B.kind === 'boodie') {
    ...
    else if (!(grab && grab.B === B) && gravDir > 0 && B.sinceHurt > 1.3 && B.pain < 70 && B.onFire <= 0 && B.zapped <= 0) B.stand = Math.min(1, B.stand + dt * .6);
...
function step(dt) {
  ...
  for (let i = parts.length - 1; i >= 0; i--) {
    ...
    q.life -= dt;
    q.x += q.vx; q.y += q.vy;
    if (q.type === 'drop' || q.type === 'spark' || q.type === 'shell' || q.type === 'tooth' || q.type === 'gib') q.vy += G * 1.1;
...
$('btnClear').onclick = () => { props = []; ents = []; nurseOnDuty = false; gravDir = 1; flipT = 0; boodie = null; refreshTools(); for (const B of bodies()) { B.decals = []; B.bleed = 0; B.swell = 0; B.pinned = new Array(14).fill(null); } floorStains = []; wallMarks = []; parts = []; grab = null; };
$('btnReset').onclick = () => {
